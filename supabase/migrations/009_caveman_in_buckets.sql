-- ai-news-feed :: surface caveman_summary in story_buckets + notable_solo_items
--
-- Pure additive: replaces both functions in place so frontend sees
-- caveman_summary on every paper member. Schema-compatible with the prior
-- shape — the new field is a nullable jsonb key inside `members`.
--
-- Safe to re-run.
